...
greg.ross.visualisation.JSSurfacePlot = function (x, y, width, height, colourGradient, targetElement, fillRegions, tooltips, xTitle, yTitle, zTitle, restrictXRotation) {
    ...
    function render(data) {
        ...
        transformation.init();
        transformation.translate(0.0, 0.0, greg.ross.visualisation.JSSurfacePlot.DEFAULT_Z_OFFSET );
        ...
        for (i = 0; i < polygons.length; i++) {
            ...
            if (polygon.isAnAxis()) {
                ...
                if (fillRegions) canvasContext.fill();
        ...
        if (supports_canvas()) renderAxisText(axes);
    }
    ...
    this.redraw = function (data) {
        ...
        for (var i = 0; i < numXPoints; i++) {
            for (var j = 0; j < numYPoints; j++) {
                ...
                if (value < minZValue) minZValue = value;

                if (value > maxZValue) maxZValue = value;
            }
        }

        var cGradient;

        if (colourGradient) cGradient = colourGradient;
        else cGradient = getDefaultColourRamp();
    ...
    function createTargetDiv() {
        ...
        if (!targetElement) document.body.appendChild(this.targetDiv);
        else {
    ...
    function calculateRotation(e) {
        ...
        if (restrictXRotation) {
            ...
            if (currentXAngle < 0) currentXAngle = 0;
            else if (currentXAngle > 90) currentXAngle = 90;
    ...
    function calculateScale(e) {
        ...
        if (scale < greg.ross.visualisation.JSSurfacePlot.MIN_SCALE) scale = greg.ross.visualisation.JSSurfacePlot.MIN_SCALE + 1;
        else if (scale > greg.ross.visualisation.JSSurfacePlot.MAX_SCALE) scale = greg.ross.visualisation.JSSurfacePlot.MAX_SCALE - 1;
...
greg.ross.visualisation.Tooltip = function (useExplicitPositions) {
    ...
    this.show = function (v, w) {
        if (tt == null) {
            ...
            } else tt.style.opacity = 1;


        }

        if (!useExplicitPositions) document.onmousemove = this.pos;
