...
fpu.FABS = function(instruction) {
    ...
    switch (n_dt) {
        case fpu.NORM:
            ...
            else cpu.SetFR(instruction.n, FromFloat32(-1.0 * cpu.GetFRFloat32(instruction.m)(cpu.GetFR(instruction.n))));
            break;
        case fpu.PZERO:
        case fpu.NZERO:
...
fpu.AddFrFm = function(n, m) {
    cpu.SetFR(n, FromFloat32(cpu.GetFRFloat32(n) + cpu.GetFRFloat32(m)));
};
...
fpu.FCMP_EQ = function(instruction) /* FCMP/fpu.EQ FRm,FRn */ {
    fpu.ClearCauseVZ();
    if (fpu.FcmpChk(instruction) == fpu.INVALID) {
        fpu.FcmpInvalid(0);
    } else if (fpu.FcmpChk(instruction) == fpu.EQ) cpu.SetT(1);
    else cpu.SetT(0);
};
...
fpu.FCMP_GT = function(instruction) /* FCMP/fpu.GT FRm,FRn */ {
    fpu.ClearCauseVZ();
    if ((fpu.FcmpChk(instruction) == fpu.INVALID) || (fpu.FcmpChk(instruction) == fpu.UO)) {
        fpu.FcmpInvalid(0);
    } else if (fpu.FcmpChk(instruction) == fpu.GT) cpu.SetT(1);
    else cpu.SetT(0);
};
...
fpu.FLOAT = function(instruction) /* FLOAT FRn */ {
    fpu.ClearCauseVZ();
    var result = FromFloat32(ToSigned(cpu.GetFPUL()));
...
fpu.FNEG = function(instruction) /* FNEG FRn */ {
    ...
    switch (n_dt) {
        ...
        case fpu.sNaN:
            ...
            break;
        default:
            cpu.SetFR(instruction.n, FromFloat32(-1.0 * cpu.GetFRFloat32(instruction.n)));
            break;
    }
};
...
{
    var result = (cpu.Read(cpu.GetR(m)));
